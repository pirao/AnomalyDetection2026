...
# Copy dependency metadata before source code. This lets Docker reuse the heavy
# third-party dependency layers when application files change but dependency
# metadata does not. README.md is required because pyproject.toml declares it as
# package metadata (`readme = "README.md"`).
...
RUN uv sync --locked --no-dev --extra notebooks --no-install-project

COPY src/sample_processing ./src/sample_processing
COPY src/analysis ./src/analysis

RUN uv sync --locked --no-dev --extra notebooks
